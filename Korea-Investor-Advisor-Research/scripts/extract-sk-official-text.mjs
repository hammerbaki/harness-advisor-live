...
const inventoryPath = resolve(process.env.SK_INVENTORY ?? join(rootDir, "raw", "manifests", "sk.local-sources.json"));
const reportPath = resolve(
  process.env.SK_EXTRACT_REPORT ?? join(rootDir, "raw", "manifests", "sk.extraction-report.json")
);
const textOutputDir = resolve(
  process.env.SK_EXTRACT_TEXT_DIR ?? join(rootDir, "raw", "extracted", "sk", "official")
);
const writeText = process.env.SK_EXTRACT_WRITE_TEXT === "1";
const selectedIds = new Set(
  String(process.env.SK_EXTRACT_IDS ?? "")
    .split(",")
    .map((id) => id.trim())
    .filter(Boolean)
);
...
const invalidPdfFiles = inventory.entries.filter((entry) => entry.processingDecision?.startsWith("exclude-"));
const urlPendingCandidates = candidates.filter((entry) => entry.documentUrlStatus !== "matched_from_document_url_list");
...
for (const entry of candidates) {
  ...
  try {
    ...
    const result = {
      ...
      promotionBoundary:
        entry.documentUrlStatus === "matched_from_document_url_list"
          ? "eligible-for-human-claim-review-after-evidence-locator"
          : "review-only-until-document-url-reconciled",
...
const report = {
  ...
  extractionPolicy:
    "Default output records extraction metadata only. Set SK_EXTRACT_WRITE_TEXT=1 for local full-text markdown review.",
  textOutputIncluded: writeText,
  promotionPolicy:
    "Extracted markdown is review material. Runtime source-backed claims require public document URL, evidence locator, period/reporting basis, and reviewer approval.",
  totals: {
    ...
    urlPendingCandidates: urlPendingCandidates.length,
    urlMatchedCandidates: candidates.length - urlPendingCandidates.length,
...
console.log(`SK extraction report written: ${relative(rootDir, reportPath)}`);
...
console.log(`${report.totals.urlPendingCandidates} extracted candidate(s) still need document URL reconciliation before claim promotion.`);
if (!writeText) console.log("Full-text markdown was not written. Set SK_EXTRACT_WRITE_TEXT=1 for local review output.");
