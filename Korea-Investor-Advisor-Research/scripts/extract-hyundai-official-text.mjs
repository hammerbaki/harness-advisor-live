...
const reportPath = resolve(
  process.env.HYUNDAI_EXTRACT_REPORT ?? join(rootDir, "raw", "manifests", "hyundai-motor.extraction-report.json")
);
const textOutputDir = resolve(
  process.env.HYUNDAI_EXTRACT_TEXT_DIR ?? join(rootDir, "raw", "extracted", "hyundai-motor", "official")
);
const writeText = process.env.HYUNDAI_EXTRACT_WRITE_TEXT === "1";
const selectedIds = new Set(
  String(process.env.HYUNDAI_EXTRACT_IDS ?? "")
    .split(",")
    .map((id) => id.trim())
    .filter(Boolean)
);
...
const report = {
  ...
  extractionPolicy:
    "Default output records extraction metadata only. Set HYUNDAI_EXTRACT_WRITE_TEXT=1 for local full-text markdown review.",
  textOutputIncluded: writeText,
  promotionPolicy:
    "Extracted markdown is review material. Runtime source-backed claims require public document URL or DART receipt, evidence locator, period/reporting basis, rights-safe source metadata, and reviewer approval.",
  totals: {
...
console.log(`Hyundai Motor extraction report written: ${relative(rootDir, reportPath)}`);
...
if (!writeText) console.log("Full-text markdown was not written. Set HYUNDAI_EXTRACT_WRITE_TEXT=1 for local review output.");
