...
npm run dev              # scripts/dev.mjs: API (server/index.mjs on :8787, override API_PORT) + Vite UI (:5173), proxied via vite.config.ts
...
- **`server/index.mjs`** — the entire runtime assembly layer in one file (~2600 lines). It resolves the selected group/company, collects source packages, assembles a source-backed answer, and emits a process trace. Three endpoints: `GET /api/healthz`, `GET /api/groups`, `POST /api/advisor`. Key flow: `runAdvisor` → tool fetches (`fetchDartDisclosures`, `fetchKrxMarket`, `searchNews`) → `loadWikiContext` + `loadSourceBackedClaims` → `composeWithLLM` (or `deterministicAnswer`/`composeDeterministicInvestorAnswer` fallback) → `validateStructuredAdvisorOutput` → `finalizeAdvisorAnswer`. Each external source has a live mode (credentialed) and a fixture/fallback mode; the trace envelope makes fixture/fallback/live runs distinguishable, which the manuscript relies on. The decisive enforcement gate is `validateStructuredAdvisorOutput` + deterministic fallback: it is what keeps leakage/recommendation/structure violations out of the reader-facing answer (the prompt-only ablation disables exactly this gate). `searchNews` and the source fixtures feed **both** `runAdvisor` and the briefing snapshot, so editing a fixture affects the eval path too.
