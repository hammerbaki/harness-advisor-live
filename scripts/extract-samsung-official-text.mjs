...
const inventoryPath = resolve(
  process.env.SAMSUNG_INVENTORY ?? join(rootDir, "raw", "manifests", "samsung.local-sources.json")
);
const reportPath = resolve(
  process.env.SAMSUNG_EXTRACT_REPORT ?? join(rootDir, "raw", "manifests", "samsung.extraction-report.json")
);
const textOutputDir = resolve(
  process.env.SAMSUNG_EXTRACT_TEXT_DIR ?? join(rootDir, "raw", "extracted", "samsung", "official")
);
const writeText = process.env.SAMSUNG_EXTRACT_WRITE_TEXT === "1";
const selectedIds = new Set(
  String(process.env.SAMSUNG_EXTRACT_IDS ?? "")
    .split(",")
    .map((id) => id.trim())
    .filter(Boolean)
);
...
const skippedDartViewer = inventory.entries.filter((entry) => entry.processingDecision === "extract-via-dart-document-pipeline");
...
const report = {
  ...
  extractionPolicy:
    "Default output records extraction metadata only. Set SAMSUNG_EXTRACT_WRITE_TEXT=1 for local full-text markdown review.",
  ...
  skipped: {
    dartViewerFilings: skippedDartViewer.length,
...
console.log(`Samsung extraction report written: ${relative(rootDir, reportPath)}`);
...
console.log(`${report.skipped.dartViewerFilings} DART viewer filing(s) require the DART document pipeline.`);
if (report.skipped.invalidPdfFiles > 0) console.log(`${report.skipped.invalidPdfFiles} invalid PDF file(s) skipped.`);
if (!writeText) console.log("Full-text markdown was not written. Set SAMSUNG_EXTRACT_WRITE_TEXT=1 for local review output.");
